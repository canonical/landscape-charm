...
class ServicesHookTest(HookenvTest):
    ...
    def test_root_url_is_set_and_invalid(self):
        """
        If an invalid root-url config option is set, an error message is
        printed in the logs.
        """
        self.hookenv.config().update({"root-url": "asd"})
        self.hook()
        self.assertIn(
            ("The 'root-url' configuration value is not a valid URL. Please"
             " make sure it is of the form 'http[s]://<hostname>/'", "ERROR"),
            self.hookenv.messages)

    def test_root_url_is_set_without_protocol(self):
        """
        If an invalid root-url config option is set without a protocol, an
    ...
    def test_root_url_is_set_without_trailing_slash(self):
        ...
        an error message is printed in the logs.
        """
    ...
    def test_a_valid_root_url_configuration_key_is_set(self):
        """
        No error message is printed if the root-url is a valid URL.
        """
        self.hookenv.config().update({"root-url": "https://example.com/"})
        self.hook()
        self.assertNotIn(
